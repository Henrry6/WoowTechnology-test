...
var LocaleProvider = (props) => {
  ...
  const getMemoizedContextValue = React10.useMemo(() => ({
    ...locale5,
    exist: true
  }), [locale5]);
...
function useDom(render, debug) {
  ...
  const appendedRef = React13.useRef(false);
  const queueCreate = React13.useContext(Context_default2);
  const [queue, setQueue] = React13.useState(EMPTY_LIST);
...
var useId_default = useOriginId ? (
  ...
  function useCompatId(id) {
    const [innerId, setInnerId] = React15.useState("ssr-id");
    React15.useEffect(() => {
      const nextId = uuid3;
      uuid3 += 1;
...
var Portal = React16.forwardRef((props, ref) => {
  const {
    open,
    autoLock,
    getContainer,
  ...
  const [innerContainer, setInnerContainer] = React16.useState(() => getPortalContainer(getContainer));
  React16.useEffect(() => {
  ...
  if (ref) {
    reffedChildren = React16.cloneElement(children, {
      ref: mergedRef
    });
  }
  return React16.createElement(Context_default2.Provider, {
    value: queueCreate
...
function Collection({
  ...
  const resizeIdRef = React17.useRef(0);
  const resizeInfosRef = React17.useRef([]);
  const onCollectionResize = React17.useContext(CollectionContext);
  const onResize2 = React17.useCallback((size, element, data) => {
...
function SingleObserver(props, ref) {
  ...
  const onCollectionResize = React19.useContext(CollectionContext);
  const isRenderProps = typeof children === "function";
  const mergedChildren = isRenderProps ? children(elementRef) : children;
  const canRef = !isRenderProps && React19.isValidElement(mergedChildren) && supportRef(mergedChildren);
  const originRef = canRef ? getNodeRef(mergedChildren) : null;
  const mergedRef = useComposeRef(originRef, elementRef);
  const getDomElement = () => {
    return getDOM(elementRef.current);
  };
  React19.useImperativeHandle(ref, () => getDomElement());
  ...
  return canRef ? React19.cloneElement(mergedChildren, {
    ref: mergedRef
  }) : mergedChildren;
}
...
var MotionProvider = (props) => {
  ...
  const memoizedValue = React21.useMemo(() => {
    return {
      motion: rest.motion
    };
  }, [rest.motion]);
...
var context_default2 = MotionProvider;

// node_modules/@rc-component/util/es/hooks/useSyncState.js
var React22 = __toESM(require_react());
function useSyncState(defaultValue) {
  const [, forceUpdate] = React22.useReducer((x) => x + 1, 0);
  const currentValueRef = React22.useRef(defaultValue);
...
var useNextFrame_default = () => {
  const nextFrameRef = React24.useRef(null);
...
function useStatus(supportMotion, visible, getElement, {
  ...
  const [asyncVisible, setAsyncVisible] = React26.useState();
  const [getStatus, setStatus] = useSyncState_default(STATUS_NONE);
  const [style2, setStyle] = React26.useState([null, null]);
  ...
  const eventHandlers = React26.useMemo(() => getEventHandlers(currentStatus), [currentStatus]);
  const [startStep, step] = useStepQueue_default(currentStatus, !supportMotion, (newStep) => {
    ...
    if (newStep in eventHandlers) {
      setStyle([((_a = eventHandlers[newStep]) == null ? void 0 : _a.call(eventHandlers, getDomElement(), null)) || null, newStep]);
    }
  ...
  const firstMountChangeRef = React26.useRef(false);
  (0, import_react12.useEffect)(() => {
    if (asyncVisible) {
...
function genCSSMotion(config) {
  ...
  const CSSMotion = React27.forwardRef((props, ref) => {
    const {
      // Default config
      visible = true,
      removeOnLeave = true,
    ...
    const status = getStatus();
    const renderedRef = React27.useRef(mergedVisible);
    if (mergedVisible) {
      renderedRef.current = true;
    }
    const refObj = React27.useMemo(() => {
      const obj = {};
    ...
    React27.useImperativeHandle(ref, () => refObj, []);
    const idRef = React27.useRef(0);
    if (styleReady) {
      idRef.current += 1;
    }
...
function genCSSMotionList(transitionSupport, CSSMotion = CSSMotion_default) {
  class CSSMotionList extends React28.Component {
    constructor() {
      super(...arguments);
    ...
    render() {
      ...
      const Component2 = component || React28.Fragment;
      const motionProps = {};
      ...
      return React28.createElement(Component2, restProps, keyEntities.map(({
        status,
        ...eventProps
      }, index) => {
        const visible = status === STATUS_ADD || status === STATUS_KEEP;
        return React28.createElement(CSSMotion, _extends2({}, motionProps, {
          key: eventProps.key,
          visible,
          eventProps,
...
function Mask(props) {
  ...
  return React30.createElement(es_default3, _extends3({}, motion, {
    motionAppear: true,
    visible: open,
    removeOnLeave: true
  }), ({
...
var Popup = React32.forwardRef((props, ref) => {
  const {
    onEsc,
    popup,
    className,
  ...
  const [mergedMask, mergedMaskMotion, mergedPopupMotion] = React32.useMemo(() => {
    if (mobile) {
  ...
  const [show, setShow] = React32.useState(!getPopupContainer || !getPopupContainerNeedParams);
  useLayoutEffect_default(() => {
  ...
  return React32.createElement(Portal2, {
    ...
    return React32.createElement(es_default3, _extends4({
      motionAppear: true,
      motionEnter: true,
      motionLeave: true,
      removeOnLeave: false,
...
var UniqueContext = React33.createContext(null);
...
function useAlign(open, popupEle, target, placement, builtinPlacements, popupAlign, onPopupAlign, mobile) {
  const [offsetInfo, setOffsetInfo] = React35.useState({
    ready: false,
    offsetX: 0,
    offsetY: 0,
    offsetR: 0,
  ...
  const alignCountRef = React35.useRef(0);
  const scrollerList = React35.useMemo(() => {
    if (!popupEle || mobile) {
      return [];
    }
    return collectScroller(popupEle);
  }, [popupEle]);
  const prevFlipRef = React35.useRef({});
...
function useDelay() {
  const delayRef = React36.useRef(null);
  const clearDelay = () => {
    if (delayRef.current) {
...
function useWinClick(open, clickToHide, targetEle, popupEle, mask, maskClosable, inPopupOrChild, triggerOpen) {
  const openRef = React37.useRef(open);
  openRef.current = open;
  const popupPointerDownRef = React37.useRef(false);
  React37.useEffect(() => {
    var _a, _b;
...
var UniqueProvider = ({
  ...
  const mergedOptions = React40.useMemo(() => {
    if (!options || !postTriggerProps) {
      return options;
    }
    return postTriggerProps(options);
  }, [options, postTriggerProps]);
  const [popupEle, setPopupEle] = React40.useState(null);
  const [popupSize, setPopupSize] = React40.useState(null);
  const externalPopupRef = React40.useRef(null);
  ...
  const isOpenRef = React40.useRef(null);
  ...
  const alignedClassName = React40.useMemo(() => {
    var _a;
    if (!mergedOptions) {
      return "";
    }
  ...
  const subPopupElements = React40.useRef({});
  const parentContext = React40.useContext(context_default3);
  const triggerContextValue = React40.useMemo(() => ({
  ...
  return React40.createElement(UniqueContext.Provider, {
    value: contextValue
  }, children, mergedOptions && React40.createElement(context_default3.Provider, {
  ...
  }, React40.createElement(UniqueContainer_default, {
    prefixCls,
    isMobile: false,
    ready,
    open,
...
function generateTrigger(PortalComponent = es_default) {
  const Trigger = React41.forwardRef((props, ref) => {
    const {
    ...
    const parentContext = React41.useContext(context_default3);
    ...
    const uniqueContext = React41.useContext(UniqueContext);
    const id = useId_default();
    ...
    const [targetEle, setTargetEle] = React41.useState(null);
    const externalForwardRef = React41.useRef(null);
    ...
    const child = React41.useMemo(() => {
      const nextChild = typeof children === "function" ? children({
        open: mergedOpen
      }) : children;
      return React41.Children.only(nextChild);
    }, [children, mergedOpen]);
    ...
    const openRef = React41.useRef(mergedOpen);
    ...
    const [inMotion, setInMotion] = React41.useState(false);
    useLayoutEffect_default((firstMount) => {
      if (!firstMount || mergedOpen) {
        setInMotion(true);
      }
    }, [mergedOpen]);
    const [motionPrepareResolve, setMotionPrepareResolve] = React41.useState(null);
    const [mousePos, setMousePos] = React41.useState(null);
    ...
    const [targetWidth, setTargetWidth] = React41.useState(0);
    const [targetHeight, setTargetHeight] = React41.useState(0);
    ...
    const touchedRef = React41.useRef(false);
    ...
    const rendedRef = React41.useRef(false);
    ...
    const triggerNode = React41.cloneElement(child, {
      ...mergedChildrenProps,
      ...passedProps,
      ref: mergedRef
    });
    return React41.createElement(React41.Fragment, null, triggerNode, rendedRef.current && (!uniqueContext || !unique) && React41.createElement(context_default3.Provider, {
      value: context
...
var ConfigContext = React43.createContext({
...
function MotionWrapper(props) {
  ...
  if (needWrapMotionProviderRef.current) {
    return React50.createElement(MotionCacheContext.Provider, {
      value: motion
    }, React50.createElement(context_default2, {
      motion
    }, children));
  }
  return children;
...
var LayoutContext = React53.createContext({
  siderHook: {
    addSider: () => null,
    removeSider: () => null
  }
...
var SiderContext = React54.createContext({});
...
var Sider = React54.forwardRef((props, ref) => {
  const {
  ...
  const {
    siderHook
  } = (0, import_react25.useContext)(LayoutContext);
  const [collapsed, setCollapsed] = (0, import_react25.useState)("collapsed" in props ? props.collapsed : defaultCollapsed);
  const [below, setBelow] = (0, import_react25.useState)(false);
  ...
  const responsiveHandlerRef = (0, import_react25.useRef)(null);
  ...
  const zeroWidthTrigger = Number.parseFloat(String(collapsedWidth || 0)) === 0 ? React54.createElement("span", {
    onClick: toggle,
    className: clsx(`${prefixCls}-zero-width-trigger`, `${prefixCls}-zero-width-trigger-${reverseArrow ? "right" : "left"}`),
    style: zeroWidthTriggerStyle
  }, trigger || React54.createElement(BarsOutlined_default, null)) : null;
  const reverseIcon = direction === "rtl" === !reverseArrow;
  const iconObj = {
    expanded: reverseIcon ? React54.createElement(RightOutlined_default, null) : React54.createElement(LeftOutlined_default, null),
    collapsed: reverseIcon ? React54.createElement(LeftOutlined_default, null) : React54.createElement(RightOutlined_default, null)
  };
  const status = collapsed ? "collapsed" : "expanded";
  const defaultTrigger = iconObj[status];
  const triggerDom = trigger !== null ? zeroWidthTrigger || React54.createElement("div", {
  ...
  return React54.createElement(SiderContext.Provider, {
    value: contextValue
  }, React54.createElement("aside", {
    className: siderCls,
    ...divProps,
    style: divStyle,
    ref
...
var generator = ({
  ...
  return (Component2) => {
    const Adapter = React55.forwardRef((props, ref) => React55.createElement(Component2, {
      ref,
      suffixCls,
      tagName,
      ...props
...
var Basic = React55.forwardRef((props, ref) => {
  const {
    prefixCls: customizePrefixCls,
    suffixCls,
    className,
  ...
  return React55.createElement(TagName, {
    className: clsx(customizePrefixCls || prefixWithSuffixCls, className, hashId),
    ref,
    ...others
  });
});
var BasicLayout = React55.forwardRef((props, ref) => {
  const {
    direction
  } = React55.useContext(ConfigContext);
  const [siders, setSiders] = React55.useState([]);
  const {
    prefixCls: customizePrefixCls,
    className,
    rootClassName,
  ...
  return React55.createElement(LayoutContext.Provider, {
    value: contextValue
  }, React55.createElement(Tag, {
    ref,
...
//# sourceMappingURL=chunk-KS6TUJBC.js.map
